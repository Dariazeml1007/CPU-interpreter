...
void CPU::execute_LD(CPU& cpu, uint32_t instr)
{
    ...
    uint32_t address = cpu.gpr[base] + offset; // from base and offset
    cpu.gpr[rt] = cpu.memory->read32(address);// to rt
}

void CPU::execute_CLS(CPU& cpu, uint32_t instr)
{
    uint8_t rd = (instr >> 21) & 0x1F;  // куда записать результат
    uint8_t rs = (instr >> 16) & 0x1F;  // откуда взять число

    uint32_t value = cpu.gpr[rs];
    uint32_t sign_bit = value >> 31;    // знаковый бит (0 или 1)

    // Считаем сколько битов в начале совпадают со знаком
    int count = 0;
    for (int i = 31; i >= 0; i--)
    {
...
void CPU::execute_SYSCALL(CPU& cpu, uint32_t instr)
{
    ...
    switch(syscall_num)
    {
        ...
        case SYS_PRINT_INT:
            ...
            std::cout << "Output: " << cpu.gpr[0] << std::endl;
            cpu.gpr[0] = 0;
            break;
...
void CPU::execute_SBIT(CPU& cpu, uint32_t instr)
{
    ...
    uint8_t rs = (instr >> 16) & 0x1F;  // спросить про rs !!!!
    int16_t imm5 = (instr >> 11) & 0x1F;
...
{
    uint32_t index = instr & 0x3FFFFFF; // 25:0

    // PC ← PC[31:28] || instr_index || 0b00
    uint32_t pc_high_bits = (cpu.pc & 0xF0000000); // Берем старшие 4 бита PC
    uint32_t target = pc_high_bits | (index << 2); // Сдвигаем индекс на 2 бита влево

    cpu.pc = target; // Прямая установка PC (не через set_pc)
}
...
void CPU::execute_ST(CPU& cpu, uint32_t instr)
{
    ...
    if ((address & 0x3) != 0) {
        std::cout << "ST: Misaligned access at address 0x" << std::hex << address << std::endl;
        return;
    }

    // Используем метод write32 из Memory
...
void CPU::execute_STP(CPU& cpu, uint32_t instr)
{
    ...
    int16_t offset = static_cast<int16_t>((instr & 0x7FF) << 1);

    int32_t sign_extended_offset = static_cast<int32_t>(offset);
    uint32_t addr = cpu.gpr[base] + sign_extended_offset;

    // Проверка выравнивания
    if ((addr & 0x3) != 0) {
        std::cout << "STP: Misaligned access at address 0x" << std::hex << addr << std::endl;
        return;
    }

    // Запись пары регистров
